...
@click.command()
@click.option('--timeout', default=100, help='Time delay between each move')
@click.option('--length', default=3, help='Initial length of the snake')
@click.option('--level-score', default=5, help='Score required to reach the next level')
def cli(timeout, length, level_score):
    ...
    while True:
        ...
        if next_direction == curses.KEY_RIGHT and direction != curses.KEY_LEFT:
            ...
            game_window.timeout(timeout)  # reset the timeout to the default value
            ...
            while True:
                ...
                if key == -1:  # no key is being pressed
                    ...
                    game_window.timeout(timeout // 2)  # decrease the timeout to make the snake move faster
